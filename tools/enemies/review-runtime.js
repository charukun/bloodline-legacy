...
(async()=>{
 ...
 try{
  ...
  const sample=t=>EnemyReview.sample(templates,{form:$('form').value,count:Number($('count').value),pose:$('pose').value,time:t});
  ...
  $('measure').onclick=()=>{if(measurement)return;paused=false;$('pause').textContent='⏸ 停止';measurement={start:performance.now(),runs:[[],[],[]],config:{version,form:$('form').value,pose:$('pose').value,count:$('count').value,quality:r.quality}};document.querySelectorAll('button,select,input').forEach(e=>e.disabled=true);};
