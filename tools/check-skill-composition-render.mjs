...
const flowOnly=process.argv.includes('--flow'),noAccents=process.argv.includes('--no-accents');
const baseline=process.argv.includes('--baseline'),readyOnly=process.argv.includes('--ready'),cases=flowOnly?[{special:'flame',label:'Cinderbrand'},{special:'lightning',label:'Storm Swallow'},{preset:3,label:'withdrawal dust'},{flow:'crawler',label:'flow crawler'},{flow:'soldier',label:'flow soldier'}]:readyOnly?[-1,0,2].map(weapon=>({weapon,ready:true,label:'ready '+weapon})):[0,1,2,3,4,5].map(preset=>({preset,label:'composition '+preset}));
if(!baseline&&!readyOnly&&!flowOnly)for(const [weapon,skill]of [[0,4001],[1,4002],[2,4003],[3,4004],[4,4005],[5,4006],[0,4016],[4,4017],[1,4019],[2,null]])cases.push({weapon,skill,label:skill?'weapon '+skill:'stowed greatsword'});
if(process.argv.includes('--samples-only'))cases.splice(3);
...
for(const spec of cases){
 if(spec.flow){change('target',spec.flow);change('weapon',0);change('preset',0);}else if(spec.special){w.document.getElementById(spec.special).click();}else if(spec.preset!==undefined)change('preset',spec.preset);else{change('weapon',spec.weapon);if(spec.skill)change('weapon-skill',spec.skill);else change('preset',0);}
 change('distance',2.5);w.document.getElementById('repeat').checked=false;if(spec.flow){lab.randomize(1741);w.document.getElementById('flow').click();w.document.getElementById('rhythm').checked=true;}else w.document.getElementById('rhythm').checked=false;
 if(noAccents){lab.getRenderer().combatPresentation.motionAccent=()=>{};lab.getRenderer().combatPresentation.drawDust=()=>{};}
 if(spec.ready)w.document.getElementById('ready').click();const times=[];let fxPeak=0,tipFrames=0,maxTravel=0,dustPeak=0,budgetMin=512;
 for(let i=0;i<(spec.flow?720:180);i++){const start=performance.now();frame(wall);if(i>10)times.push(performance.now()-start);wall+=1000/60;
  const r=lab.getRenderer(),p=lab.getTrial().p;dustPeak=Math.max(dustPeak,r.combatPresentation.dust?.length||0);budgetMin=Math.min(budgetMin,r.combatPresentation.compositionBudget);fxPeak=Math.max(fxPeak,[...r.fxBatches,...r.arcaneFX||[]].filter(([k])=>k.startsWith('skillfx:')||k.startsWith('trail:')).reduce((sum,[,v])=>sum+v.length,0));if(r.weaponTips.has(p.id))tipFrames++;maxTravel=Math.max(maxTravel,Math.hypot(p.x,p.z));
  for(const [key,rows]of [...r.fxBatches,...r.arcaneFX||[]]){if(rows.some(row=>!Array.from(row).every(Number.isFinite)))throw Error('invalid FX row '+spec.label+' '+key);if(key.startsWith('skillfx:')){const g=w.__review.rGeometry(key);if(!g.positions.every(Number.isFinite)||!g.normals.every(Number.isFinite))throw Error('invalid FX mesh '+spec.label+' '+key);}}
  if(!r.characterMaster.palette.every(Number.isFinite))throw Error('invalid palette '+spec.label);
  if(flowOnly?i%6===0:spec.ready?i%6===0:[20,65,120].includes(i)){const r=lab.getRenderer(),cm=r.characterMaster;asset={attrs:Object.fromEntries(Object.entries(cm.asset.lods[0].attrs).map(([k,v])=>[k,Array.from(v)])),indices:Array.from(cm.asset.lods[0].indices)};const parts=[];
   ...
   const fx=[];if(flowOnly)for(const [type,rows]of [...r.fxBatches,...r.arcaneFX||[]]){if(!type.startsWith('skillfx:'))continue;const g=w.__review.rGeometry(type);for(const row of rows)fx.push({positions:Array.from(g.positions),uv:Array.from(g.normals),m:row.slice(0,16),ink:row.slice(16,20),surface:row[20],additive:r.arcaneFX?.has(type)});}
   poses.push({fx,t:lab.getTrial().sim.time,label:spec.label,p:{x:lab.getTrial().p.x,z:lab.getTrial().p.z},palette:Array.from(cm.palette),parts});
  }}
 const r=lab.getRenderer();times.sort((a,b)=>a-b);captures.push({label:spec.label,stats:r.stats,hits:lab.getTrial().sim.events.filter(e=>e.type==='hit').length,fxPeak,tipFrames,maxTravel,dustPeak,budgetMin,uniqueSkills:new Set(lab.getTrial().sim.events.filter(e=>e.type==='skill').map(e=>e.id)).size,cpuMedianMs:times[Math.floor(times.length*.5)],cpuP95Ms:times[Math.floor(times.length*.95)]});if(!spec.ready&&!fxPeak)throw Error('No skill VFX '+spec.label);if(spec.skill&&!tipFrames)throw Error('No held weapon path '+spec.label);if(spec.ready&&(maxTravel>1e-5||r.characterMaster.metrics.animation!=='combat_idle'))throw Error('Stance preview moved the actor or lost combat stance');
}
...
fs.writeFileSync(root+'dist/composition-poses.json',JSON.stringify({asset,geometry,frames:poses,fragment:w.__review.RFRAG}));dom.window.close();
