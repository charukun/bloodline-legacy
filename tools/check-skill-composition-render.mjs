...
const root=process.argv[2]||fileURLToPath(new URL('../',import.meta.url)),html=fs.readFileSync(root+'dist/Bloodline_Legacy_Skill_Composition.html','utf8');
...
const baseline=process.argv.includes('--baseline'),cases=[0,1,2,3,4,5].map(preset=>({preset,label:'composition '+preset}));
if(!baseline)for(const [weapon,skill]of [[0,4001],[1,4002],[2,4003],[3,4004],[4,4005],[5,4006],[0,4016],[4,4017],[1,4019],[2,null]])cases.push({weapon,skill,label:skill?'weapon '+skill:'stowed greatsword'});
...
for(const spec of cases){
 ...
 change('distance',2.5);const times=[];let fxPeak=0,tipFrames=0,maxTravel=0;
 for(let i=0;i<180;i++){const start=performance.now();frame(wall);if(i>10)times.push(performance.now()-start);wall+=1000/60;
  ...
  if([20,65,120].includes(i)){const r=lab.getRenderer(),cm=r.characterMaster;asset={attrs:Object.fromEntries(Object.entries(cm.asset.lods[0].attrs).map(([k,v])=>[k,Array.from(v)])),indices:Array.from(cm.asset.lods[0].indices)};const parts=[];
 ...
 const r=lab.getRenderer();times.sort((a,b)=>a-b);captures.push({label:spec.label,stats:r.stats,hits:lab.getTrial().sim.events.filter(e=>e.type==='hit').length,fxPeak,tipFrames,maxTravel,cpuMedianMs:times[Math.floor(times.length*.5)],cpuP95Ms:times[Math.floor(times.length*.95)]});if(!fxPeak)throw Error('No skill VFX '+spec.label);if(spec.skill&&!tipFrames)throw Error('No held weapon path '+spec.label);
}
