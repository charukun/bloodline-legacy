// Captures actual traveler pose palettes/equipment for OFFLINE review, not browser evidence.
import fs from 'node:fs';
import {motionRuntime} from '../tests/skill-motion-harness.mjs';
const [out,revision='',raceArg='0',key='golden']=process.argv.slice(2);
if(!out)throw Error('Usage: node tools/capture-character-motion.mjs OUTPUT.json [REV] [RACE] [CLIP]');
const api=await motionRuntime(revision||null,null,true),r=api.renderer(),art=new api.VillageArt(r);
...
for(let i=0;i<=Math.ceil(seq.duration*hz);i++){
 ...
 const p=sample.p;Object.assign(p,{race:Number(raceArg),gender:[0,1,0,1][Number(raceArg)]});r.frame++;r.currentTime=sample.t;r.rigs.active.length=0;
...
const target=api.Travelers.prepare(Number(raceArg)).lods[0];const asset={attrs:Object.fromEntries(Object.entries(target.attrs).map(([k,v])=>[k,Array.from(v)])),indices:Array.from(target.indices)};
fs.writeFileSync(out,JSON.stringify({asset,hz,key,duration:seq.duration,revision:revision||'working-tree',race:Number(raceArg),geometry,frames}));
