...
for(let i=0;i<54;i++){
 const t=i/36;g.fillStyle='#111b20';g.fillRect(0,0,1080,460);
 Stage.draw(g,540,430,recipe,t,{label:'BEFORE / FACETED',legacyBlade:true});
 g.save();g.translate(540,0);Stage.draw(g,540,430,recipe,t,{label:'AFTER / SILK SURFACE'});g.restore();
 g.fillStyle='#c8b698';g.font='12px sans-serif';g.fillText('SAME RECIPE / SAME PLAYBACK CLOCK',22,450);
...
for(const path of Object.keys(FX.options.path))for(const u of [.3,.6,.85]){
 ...
 frames.push({path,u,geometry:ribbons.map(p=>{const geo=Silk.geometry(p);return {positions:[...geo.positions],normals:[...geo.normals],center:geo.center,alpha:p.alpha};})});
}
