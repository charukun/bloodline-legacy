...
export function compileCatalog(data){
 const result=[];
...
export function validateCatalog(defs){
 ...
 for(const d of defs){
  ...
  }else if(!['regen','capRegen','clash','heavyCost'].includes(d.effect)||!Number.isFinite(d.value)||d.value<0||d.value>({regen:1,capRegen:.25,clash:.15,heavyCost:.2}[d.effect]??0))throw Error('Unknown passive operator');
 }
 return true;
}
