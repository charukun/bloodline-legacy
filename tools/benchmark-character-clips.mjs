...
if(!/^[0-9a-f]{40}$/.test(base||'')||!output)throw Error('Usage: node tools/benchmark-character-clips.mjs BASE_SHA OUTPUT.json');
...
for(const [label,revision]of [['before',base],['after',null]]){
 const a=await motionRuntime(revision);
 ...
 for(let round=0;round<4;round++){
  const r=a.renderer(),cm=new a.CM01.Character(r);cm.groundAt=()=>0;
