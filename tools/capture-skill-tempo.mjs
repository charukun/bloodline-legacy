...
for(let i=0;i<600;i++){
 ...
 // Recenter the captured scene for the offline floor; retain all relative travel.
 snapshot.z+=38;r.frame++;r.currentTime=sim.time;r.rigs.active.length=0;art.doll(snapshot,sim.time,true);
 ...
 frames.push({t:sim.time,label:(api.skillById(p.attackSkill)?.name||'')+' / '+p.action+(p.skillCast?.linked?' / CONNECTED':''),p:{x:snapshot.x,z:snapshot.z,action:p.action},palette:Array.from(cm.palette),transforms:cm.transforms.map(m=>Array.from(m)),parts,clip:cm.clipDebug,feet:cm.footDebug,cast:snapshot.skillCast,attackSkill:p.attackSkill});
