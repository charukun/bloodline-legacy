...
const CompositionLab=(()=>{
 ...
 const trialSkills=[...native,skillById(60073)];
 ...
 let selected={...base,...examples[0][1]},slots=[],weapon=-1,trial,renderer,paused=false,last=0,carry=0,seq=0,hits=0,links=0,mode='single',cycleEnded=false,request,travel=0,history=[],flowPool=[[],[],[]],nextSpark=1.1;
 ...
 function def(r){
  if(r?.skillId!==undefined){const sk=trialSkills.find(s=>s.id===r.skillId);if(!sk)throw Error('稽古帳にない武器技です');const authored=BL_SKILL_CATALOG.byId.get(sk.id);if(authored)return authored;return {id:sk.id,names:{ja:sk.name,en:english[sk.id]||''},descriptions:{ja:sk.desc},phase:skillPhase(sk),entry:[],exit:[],action:sk};}
 ...
 function update(){
  ...
  $('weapon-skill').replaceChildren(...[{id:'',name:'構成技を使う'},...available(weapon)].map(s=>{const o=document.createElement('option');o.value=s.id;o.textContent=s.name;return o;}));$('weapon-skill').value=d.action.weapon<0||d.composition?'':d.id;
  $('parts').disabled=!d.composition;for(const key of Object.keys(SkillComposition.tables))$(key).value=(d.composition||base)[key];
  $('equipment-note').textContent=d.composition?(weapon<0?'拳・掌・脚で放つ構成技。':'構成技では武器を背に預け、拳・掌・脚を使います。'):d.action.unarmed?'全身を使って飛び込み、着地から身を引きます。':WEAPONS[weapon].name+'を手に持ち、武器の先端から軌跡を描きます。';
 ...
 function randomize(seed){if(Number.isInteger(seed))drawSeed=seed>>>0;remember();const old=def(selected).id;do{const accents=[60073,...weapon===0?[60093]:[]];selected=mode==='flow'&&random()<.24?{skillId:pick(accents)}:generate();}while(def(selected).id===old);update();if(mode==='flow'){addFlowSkill(selected);return;}fitDistance();launch('single');}
 ...
 function showFlow(){
  $('flow-pool').textContent=mode==='flow'?flowPool.map((pool,i)=>PHASES[i]+'：'+pool.map(r=>def(r).names.ja).join(' / ')).join(' ｜ '):'';
 }
 function addFlowSkill(recipe){
  const {sim,p}=trial,d=def(recipe),pool=flowPool[d.phase];
  if(!pool.some(r=>def(r).id===d.id)){pool.push(canonical(recipe));if(pool.length>3)pool.shift();}
  sim.learn(p,d.id);p.phaseWeights[d.phase]=Object.fromEntries(pool.map(r=>[def(r).id,1]));showFlow();
 }
 function showcase(id){remember();selected={skillId:id};const d=def(selected);
  if(d.action.weapon>=0&&weapon!==d.action.weapon){weapon=d.action.weapon;slots=slots.map((r,i)=>def(r).action.weapon>=0&&def(r).action.weapon!==weapon?generate(i):r);if(mode==='flow')launch('flow');}
  update();if(mode==='flow'){addFlowSkill(selected);return;}fitDistance();launch('single');
 }
 ...
 function clearVisual(){renderer.effects.length=0;renderer.skillMotionStates?.clear();renderer.art.skillFeet?.clear();renderer.damageMotion?.actors.clear();const fx=renderer.combatPresentation;for(const id of fx.trails.keys())fx.forget(id);fx.castSamples.clear();fx.resetMotion?.();fx.castSerial=0;fx.clearSilk?.();}
 ...
 function launch(nextMode=mode){if(!renderer)return;mode=nextMode;trial=prepareTrial({target:$('target').value,distance:+$('distance').value,weapon});seq=trial.sim.seq;hits=0;links=0;travel=0;carry=0;cycleEnded=false;clearVisual();renderer.combatPresentation.previewEnabled=$('effects').checked;
  ...
  showFlow();
  if(mode==='flow'){flowPool=[[],[],[]];p.phaseWeights=[{},{},{}];for(let phase=0;phase<3;phase++){addFlowSkill(slots[phase]);for(let j=0;j<2;j++)addFlowSkill(generate(phase));}addFlowSkill({skillId:60073});if(weapon===0)addFlowSkill({skillId:60093});addFlowSkill(selected);p.autoFight=room.actors[0].id;nextSpark=sim.time+1.1;paused=false;$('pause').textContent='一時停止';return;}
 ...
 function advance(dt){const {sim,p}=trial,x=p.x,z=p.z;sim.tick(dt);travel+=Math.hypot(p.x-x,p.z-z);
  ...
  if(mode==='flow'&&$('rhythm').checked&&sim.time>=nextSpark&&p.alive&&trial.room.actors.some(e=>e.alive)){nextSpark=sim.time+1.1;randomize();}
  if(mode!=='flow'&&!p.combo&&!p.pendingSkill&&!cycleEnded){cycleEnded=true;p.autoFight=null;p.autoSuppressedUntil=1e9;}
 }
 function restore(value){
  ...
  for(const r of [next,...nextSlots])if(def(r).action.weapon>=0&&def(r).action.weapon!==nextWeapon)throw Error('持つ武器と技が一致しません');
 ...
 async function start(){
  ...
  $('adopt').onclick=()=>{slots[def(selected).phase]=canonical(selected);if(mode==='flow')addFlowSkill(selected);update();};$('cast').onclick=()=>launch('single');$('combo').onclick=()=>launch('combo');$('pause').onclick=()=>{paused=!paused;$('pause').textContent=paused?'再生':'一時停止';};
  $('ready').onclick=()=>launch('ready');$('flow').onclick=()=>launch('flow');$('flame').onclick=()=>showcase(60093);$('lightning').onclick=()=>showcase(60073);
  $('effects').onchange=()=>{renderer.combatPresentation.previewEnabled=$('effects').checked;clearVisual();};$('fit-distance').onclick=()=>{fitDistance();launch();};$('distance').oninput=()=>{$('distanceText').value=(+$('distance').value).toFixed(1);};$('distance').onchange=()=>launch();$('target').onchange=()=>launch();
  ...
  renderer.art.parentScene=()=>{};renderer.art.sources=[];$('loading').hidden=true;$('build').textContent='2,304構成・6武器・炎剣と雷渡り · '+BUILD_INFO.shortCommit+' · 稽古用に技を開放';launch();
  ...
  function frame(now){request=requestAnimationFrame(frame);if(document.hidden){last=0;return;}const dt=last?Math.min(.05,(now-last)/1000):0;last=now;
   ...
   $('readout').textContent=(mode==='flow'?'連続戦闘 · ':'')+(active?.name||'構え')+' ／ 命中 '+hits+' · 接続 '+links+' · 移動 '+travel.toFixed(1)+' ／ '+(incapacitated(p)?'行動不能':({charge:'溜め',attack:'攻撃',recover:'立て直し',idle:'構え'})[p.action]||p.action)+($('effects').checked?' ／ '+fx:' ／ 演出OFF');
   if(!paused&&sim.time>(mode==='flow'?20:7)&&$('repeat').checked)launch();
  }request=requestAnimationFrame(frame);
  window.CompositionLab={getRecipe:()=>canonical(selected),getSlots:()=>slots.map(canonical),getTrial:()=>trial,getRenderer:()=>renderer,prepareTrial,randomize,randomCombo,launch,getFlowPool:()=>flowPool.map(pool=>pool.map(canonical)),pause:()=>{paused=true;},step:advance,dispose:()=>{cancelAnimationFrame(request);renderer.gl.getExtension('WEBGL_lose_context')?.loseContext();}};
 }
