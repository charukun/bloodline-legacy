...
Base: `957362cc74bcb6a9d8d03791dfb3dcf74f37e111` (latest develop fetched 2026-09-09).
Branch: `work/character-age-continuity-20260909`. Target: `develop`; no merge by Character WORK.
...
One rest mesh per race, two existing LODs, 16 bones (fox: 17), at most four weights. Geometry and texture buffers are not rebuilt on birthdays or growth frames. No new textures, geometry counts, draw calls, postprocessing or cloth simulation are added to the traveler. Lower screen coverage can select the existing lower LOD sooner for small children. The standalone build grew by approximately **5.5 KB** (about 0.03%). The two-entry CPU model cache and single active GPU character are retained.
...
- `npm test`: 453/453 PASS, including native GLSL ES compile/link and new age-continuity/ground-contact tests.
...
- Native browser age/model, input, release and save/reload checks: **PASS**. Captures and relative software timing: see `evidence/browser-report.json` (populated by `tests/character-age-browser.mjs`).

Reproduce browser evidence after building:

```sh
CHARACTER_BASELINE=/path/to/base/dist/index.html \
CHARACTER_BASE_SHA=957362cc74bcb6a9d8d03791dfb3dcf74f37e111 \
node tests/character-age-browser.mjs
```

The timing comparison uses nine completed frames after three warm-up frames, synchronizing the test with one-pixel readback. Before/after run sequentially at the same viewport, camera, village, medium quality and clear weather. Completed-frame medians in this software run were: age 4 **1695.9 → 1320.0 ms**, age 24 **1433.3 → 1509.4 ms**, age 80 **1432.2 → 1348.8 ms**. The adult sample was 5.3% slower; this short software run does not establish a zero-cost change on hardware. Draw calls stayed **40**; scene triangles changed from **165,510 / 168,050 / 169,886** to **168,050** in each respective age scene (about +1.5% / 0% / −1.1%).
