...
BASE_COMMIT: `6cdbacdb627bd10bccf9fb4a06f771881cc5dd3a`

BRANCH: `work/traveler-expression-injury-20260909`
...
- `npm test`: **490 tests PASS**。状態優先度・治癒への遷移・両LODの顔領域・4種族×5負傷部位×武器有無の接地・snapshot不変性・攻撃への制御引き渡しを含む。
- Native GLSL ES: 既存12プログラムのcompile/link PASS。
- Deployment build / integrity PASS。
- 本編ブラウザー: **27 checks PASS**。確認結果: [browser.json](evidence/browser.json)。CPU比較: [cpu.json](evidence/cpu.json)。
- 新規テクスチャ・頂点・三角形・骨・描画passなし。LODと単一GPU characterを維持。追加は小さな描画状態と3個のuniform。
- 独立したCPU比較3組のpose更新中央値: Before **0.213〜0.297 ms** / After **0.257〜0.435 ms**。p95はBefore **0.516〜0.774 ms** / After **0.815〜2.223 ms**。CPUの増分とtailのばらつきは残る。描画回数は同条件の通常/負傷で26→26、scene trianglesは62,362→62,362。睡眠時の既存状態VFXはこの比較に含めない。初回計測は他検証と同時に実行してしまったため、全検証終了後に上記の独立計測へやり直した。
- standalone増分は約**5.7 KB**。Pixel Fold実機のGPU性能は未確認。CPU比較はWebGL呼び出しをstubした計測で、実機FPSの保証に使わない。

```sh
npm test
CHARACTER_BASELINE=/path/to/base/dist/index.html \
CHARACTER_BASE_SHA=6cdbacdb627bd10bccf9fb4a06f771881cc5dd3a \
...
年齢継続PR #56は、このbranch開始時点のdevelopには未統合。#56との統合では、**元の顔座標 → travelerFace → agePosition → skinning** の順にする。法線もface変形後にageNormalへ渡す。#56の年齢別bind/頭部比率・抱っこ・武器年齢制限と、現在developの被弾profileを維持する。眉・目・口のregionは20〜26で、#56の髭region13とは重ならない。年齢別の完成モデルを古いdevelopで上書きしない。
