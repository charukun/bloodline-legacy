...
It outputs bounded world-space line/shard primitives plus continuous ribbons for
the blade quality pass (see [SILK_SLASH.md](SILK_SLASH.md)). There are six genuinely
...
No per-skill geometry cache, polling loop or game save extension.
Render needles and boxes reuse the established mesh resources. The blade pass
uses a bounded shared pool of 16 ribbon mesh slots (8 on low), released on room
change; it allocates no new textures. The sample's
...
Initial composer validation on 2026-09-09: 236/236 repository tests passed, including eight focused
