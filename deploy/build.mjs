...
export async function build(environment, commit, projectRoot = root) {
  ...
  let html;
  if (mode === 'game') {
    execFileSync(process.execPath, ['build.mjs'], {cwd:projectRoot, stdio:'inherit'});
    ...
    html = '<!doctype html><html lang="ja"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Bloodline Legacy</title><body><h1>Bloodline Legacy / 血脈の系譜</h1><p id="release-pending">正式公開の準備中です。</p></body></html>';
  }
  // Game bytes, UI, localStorage keys and embedded assets are not rewritten.
  const bytes = Buffer.from(html);
  await fs.writeFile(path.join(out, 'index.html'), bytes);
  const version = {application:'Bloodline Legacy', environment, branch:environments[environment].branch,
    commit, mode, htmlSha256:sha256(bytes), htmlBytes:bytes.length};
