...
function programs(){
 ...
 for(const file of ['src/render/tilt-shift.js','src/render/shaders.js','src/weather/weather.js','src/character/golden-master.runtime.js','src/character/traveler-age.js','src/character/traveler-expression.js','src/character/traveler-runtime.js'])vm.runInContext(read(file),context,{filename:file});
 ...
 result.push(...vm.runInContext(`[
  ...
  {name:'traveler/material',vertex:Travelers.shaders.VS,fragment:Travelers.shaders.FS},
  {name:'traveler/depth',vertex:Travelers.shaders.VS,fragment:Travelers.shaders.DS}
 ]`,context));
 ...
 assert.equal(result.length,12);return result;
}
...
test('all shipped renderer programs compile and link as original GLSL ES 300',nativeOptions,()=>{
 const r=compile(programs());assert.equal(r.status,0,JSON.stringify(r));assert.equal(r.ok,true);assert.equal(r.programs.length,12);
