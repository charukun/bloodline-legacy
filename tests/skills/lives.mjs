...
const api=runtime(),out={conditions:'Simulation at 30Hz; adult/location fixture; actual activity/pickup/weights/combat commands, no discovery grants. Same 720-second activity budget at the slower discovery pace; event-only diversity cohorts retain their 240-second budget.',lives:[],diversity:[],matchups:[]};
...
for(let i=0;i<plans.length;i++){
 ...
 for(const [activityId,seconds] of work)activity(sim,p,activityId,seconds*3);
...
const x=life(api,115);x.sim.receiveItem(x.p,'stone');activity(x.sim,x.p,'observe',360);activity(x.sim,x.p,'study',360);activity(x.sim,x.p,'care',360);
