...
function fixture(t, uiSource) {
 ...
 Object.assign(g,{sim,playerId:p.id,profile:{owner:p.owner,name:p.name,clan:'暁風',inherit:[],sound:false,quality:'auto'},screen:'game',online:false,loadedMode:'normal',seq:sim.seq,mapCache:new Map(),keys:new Set(),input:{x:0,z:0},walkTarget:null,audio:{enable(){},disable(){}},renderer:{canvas:w.document.getElementById('world'),labels:[],project:(x,y,z)=>({x:180+x*5,y:300-y*10,visible:true}),screenToWorld:(x,y)=>({x,z:y}),pointToWorld:(x,y)=>({x,z:y}),setQuality(){}},saveProfile(){},exportSave(){},importSave(){}});
 g.snapshot=g.decorate(sim.snapshot(p.id,sim.seq));g.ui=new UI(g);g.ui.showGame();g.ui.update(g.snapshot);
