...
const sourceFiles = ['live/contract.mjs','legacy/dialogue.js','legacy/core.js','skills/engine.js','skills/runtime.js','legacy/motion.js','ui/presentation.js','ui/lineage/theatre.js','ui/lineage.js','ui/talk-fan.js','legacy/ui.js','legacy/game.js','account/client.js','skills/presentation.js'];
...
function fixture(t, uiSource) {
 ...
 const code='const LINEAGE_VIEW='+JSON.stringify(view)+';\n'+compiler+'\nconst BL_SKILL_DEFINITIONS=compileCatalog('+catalog+');\n'+sourceFiles.map(n=>n==='legacy/ui.js'&&uiSource?uiSource:fs.readFileSync(path.join(root,'src',n),'utf8').replace(/^export /gm,'')).join('\n');
 ...
 w.eval(code+'\nwindow.testAPI={UI,Game,Simulation,AccountLink,LiveContract,STORAGE_PREFIX,facilityStation,atFacilityStation,uiJourneyHints,MOTHER_LINES,UINoticeQueue,uiCondition,uiPickup,uiActiveStatuses,EQUIP_AGE,MAX_ITEMS};');
 ...
 g.station=s=>{const a=w.testAPI.facilityStation(s);return {x:a.x,z:a.z+1};};
 g.makeClanPreview();g.snapshot=g.decorate(sim.snapshot(p.id,sim.seq));g.ui=new UI(g);g.ui.showGame();g.ui.update(g.snapshot);
 t.after(()=>{g.ui.talkFan.destroy();g.ui.lineageView.home?.destroy();g.ui.lineageView.modalView?.destroy();w.clearTimeout(g.ui.noticeTimer);dom.window.close();});
