...
export async function motionRuntime(revision=null,assetPath=null,travelers=false){
 ...
 if(travelers){
  for(const file of ['character/traveler-model.js','character/traveler-clip-data.js','character/traveler-clips.js','character/traveler-expression.js','character/traveler-runtime.js']){
   let source;try{source=revision?execFileSync('git',['show',revision+':src/'+file],{encoding:'utf8',stdio:['ignore','pipe','ignore']}):fs.readFileSync(new URL('../src/'+file,import.meta.url),'utf8');}catch(e){if(file.includes('clip')||file.includes('traveler-expression'))continue;throw e;}
