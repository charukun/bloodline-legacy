...
test('memory tokens inspect without committing, then the chosen seal is the only inherited skill',t=>{
 const {sim,p,d,sync}=fixture(t);for(const id of [4001,4100,60900])sim.learn(p,id);sim.die(p,'寿命');sync();
...
test('scrolling a large memory collection preserves the inspected skill through snapshot refresh',t=>{
 const {sim,p,d,w,ui,g,sync}=fixture(t);for(let id=4000;id<4400&&p.skills.length<40;id++)sim.learn(p,id);sim.die(p,'寿命');sync();
...
test('online confirmation sends once, can recover after a missing reply and finishes from authoritative state',t=>{
 const {sim,p,g,ui,d,w,sync}=fixture(t);sim.learn(p,4001);sim.die(p,'寿命');sync();g.online=true;g.live={ready:true};g.commandBuffer=[];
...
test('rejected confirmation and leaving the panel cannot accidentally record a skill',t=>{
 const {sim,p,g,ui,d,sync}=fixture(t);sim.learn(p,4001);sim.die(p,'寿命');sync();d.querySelector('[data-bequest="4001"]').click();g.command=()=>false;
