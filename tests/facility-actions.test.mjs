...
test('facility activity buttons appear below the player and still start/stop the real activity',t=>{
 ...
 Object.assign(p,g.station(school));sync();const group=d.querySelector('[data-anchor=feet]'),button=group.querySelector('[data-context=activity]');
...
test('armory actions share one anchor and preserve rack distance and age validation',t=>{
 const {p,g,d,sync}=fixture(t),school=g.snapshot.map.schools.find(s=>s.id==='armory');Object.assign(p,g.station(school));p.age=6;sync();
...
test('facility transforms follow fractional camera movement without changing labels or allocating nodes',t=>{
 const {p,g,ui,d,w,sync}=fixture(t),school=g.snapshot.map.schools.find(s=>s.id==='church');Object.assign(p,g.station(school));sync();
...
test('facility buttons hide immediately out of range, offscreen, in menus, and on death',t=>{
 const {p,g,ui,d,sync}=fixture(t),school=g.snapshot.map.schools.find(s=>s.id==='magic');Object.assign(p,g.station(school));sync();const node=d.querySelector('[data-anchor=feet]');
 p.x+=20;ui.positionFacilityActions(g.snapshot);assert.equal(node.style.display,'none');p.x=g.station(school).x;
...
test('facility projection does not mutate the simulation or saved state',t=>{
 const {p,g,ui,sim,sync}=fixture(t),school=g.snapshot.map.schools.find(s=>s.id==='hunter');Object.assign(p,g.station(school));sync();const before=JSON.stringify(sim.exportState());
