...
test('fence crossing uses the same landing at 30, 60 and 120 simulation steps',()=>{
 const ends=[];for(const fps of [30,60,120]){const {sim,p}=setup();let maxY=0,started=false;
  for(let i=0;i<fps*2.5;i++){sim.command(p.id,{type:'move',x:0,z:-1});sim.tick(1/fps);maxY=Math.max(maxY,p.verticalOffset||0);started||=!!p.traversal;}
  assert.ok(started,String(fps));assert.ok(maxY>.5);assert.ok(p.z<-28.5);assert.equal(p.grounded,true);assert.equal(p.traversal,null);ends.push(sim.events.find(e=>e.type==='landed').z);
...
test('three low supported steps can be climbed and descended without floating',()=>{
 ...
 walk(sim,p,0,-1,3.8);assert.ok(p.supportHeight>=.6);assert.ok(sim.events.some(e=>e.type==='traverse'&&e.kind==='climb'));
 walk(sim,p,0,1,5.4);assert.equal(p.traversal,null);assert.equal(p.supportHeight,0);assert.equal(p.verticalOffset,0);
});
...
test('diagonal approach, corners, normal wall sliding and repeated input stay bounded',()=>{
 const {sim,p,room}=setup();p.x=11.6;p.z=-27.2;assert.equal(sim.tryTraversal(p,room,.2,-1),true);walk(sim,p,1,-1,1.6);assert.ok(p.x<14&&p.z<-28.5);
...
test('injury, childhood carry, rescue carry and incapacitation cannot initiate traversal',()=>{
 for(const block of ['arm','leg','prologue','rescue','downed','root']){const {sim,p,room}=setup();p.z=-27.2;
  if(block==='arm')p.wounds.rightArm={severity:'lost'};if(block==='leg')p.wounds.leftLeg={severity:'heavy'};if(block==='prologue')p.prologue=true;if(block==='rescue')p.rescueTarget='somebody';if(block==='downed')sim.downPlayer(p,'深手');if(block==='root')sim.applyStatus(p,'root',3,null,room);
