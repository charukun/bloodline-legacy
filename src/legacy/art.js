...
class ArtDirector{
 ...
 statuses(p,t){if(!p.alive)return;const old=this.target;this.target=this.r.fxBatches;this.root=rModel();const top=this.r.enemySentinels?.records.has(p.id)?EnemySentinel.top(p):p.elite?3.5:p.kind==='crawler'?1.5:2.7;
