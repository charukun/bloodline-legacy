...
class UI {
 ...
 update(s,world=true){
  ...
  this.updateContext(s);if(world)this.updateWorld(s);if(this.modal==='hints')this.paintHints(s);
 ...
 updateContext(s){const p=s.player,t=s.t,school=nearbyActivity(p,{...s.room,map:s.map}),item=s.room.items?.find(i=>i.ready<=t&&Math.hypot(i.x-p.x,i.z-p.z)<2.3),dummy=s.actors.find(a=>a.kind==='dummy'&&a.alive&&Math.hypot(a.x-p.x,a.z-p.z)<4),options=[];
 ...
 contextAvailable(o,s){
  ...
  if(o.id==='activity'){const a=s.room.kind==='village'&&s.map.schools.find(a=>a.id===o.facility);return !!a&&p.age>=4&&atFacilityStation(p,a);}
