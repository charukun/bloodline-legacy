...
class UI {
 ...
 selectSkill(id){
  ...
  const weights={...p.phaseWeights[this.phase]};weights[id]=weights[id]>0?0:20;
 ...
 death(p){
  const key=p.id+':'+(p.legacyChoice?.state||'recorded');if(this.deathShown===key)return;this.deathShown=key;this.g.stopInput();clearTimeout(this.bequestTimer);
  ...
  this.open('death','命の灯',`<div class="death-identity"><h3 class="death-name">${ESC(p.name)}</h3><p class="death-age">享年 ${Math.floor(p.age)}歳</p></div>${pending?`<p class="bequest-invitation">ひとつの技を、次の命へ</p><div class="bequest-rite"><div class="bequest-altar"><div class="bequest-memory" id="bequest-memory">${icon('leaf')}</div><div id="bequest-detail" aria-live="polite"><strong>遺す技を選ぶ</strong><p></p></div></div><div class="legacy-bequest" role="group" aria-label="後世へ遺す技" style="--bequest-columns:${Math.max(1,Math.min(4,p.legacyChoice.candidates.length))};--bequest-mobile-columns:${Math.max(1,Math.min(3,p.legacyChoice.candidates.length))}">${choices}</div><button id="bequest-confirm" class="begin-button" disabled>${icon('leaf')}<span>この技を遺す</span>${icon('arrow')}</button></div>`:`<div class="death-leaf">${icon('leaf')}</div><p>${sk?`「${ESC(sk.name)}」を、系譜に刻んだ。`:'静かな灯が、系譜に残った。'}</p><button id="next-life" class="begin-button">次の人生へ${icon('arrow')}</button><button class="full-button" id="view-lineage">系譜をひらく</button>`}`);
  if(pending){
 ...
 event(e){
  ...
  if(e.type==='death')this.notify({key:'death',parts:['後世へ遺す技を選ぶ'],priority:4});
