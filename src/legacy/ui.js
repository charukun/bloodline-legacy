...
class UI {
 ...
 toast(text){const message=uiNoticeText(text);if(message)this.notify({key:'toast:'+message,parts:[message],priority:1});}
 notify(notice){this.notices.tick(performance.now());this.notices.enqueue(notice,performance.now());this.paintNotices();}
 paintNotices(){this.notices.tick(performance.now());uiPaintNotice(document.getElementById('toasts'),this.notices.active,this.notices.text());const until=this.notices.active?.until||0;if(until!==this.noticeTimerAt){clearTimeout(this.noticeTimer);this.noticeTimerAt=until;if(until)this.noticeTimer=setTimeout(()=>this.paintNotices(),Math.max(1,until-performance.now()));}}
 ...
 mountDock(enabled){
  ...
  UIValue.attr(this.hud.querySelector('#hud-hints'),'aria-expanded',section==='hints');
  if(this.hudSettings){
 ...
 closeModal(){
  ...
  if(this.hudDock)this.hud.appendChild(this.hudDock);
  this.restoreSettings();
  this.root.replaceChildren();this.root.className='modal-root';this.mountDock(false);this.g.live?.renderNotice?.();
 ...
 onboarding(done,index=0){const pages=[
  ...
  {title:'意識',glyph:'leaf',text:'敵に近づくと、覚えた技で戦う。<br>「序・破・急」の輪を動かして配分。<br>◆が多い技ほど、疲れやすい。',note:'意識を調整している間も、戦闘は続きます。'}
 ...
 describeSkill(id){
  ...
  const restriction=!sk.passive&&uiSkillRestrictionText(p,sk);if(restriction){const note=document.createElement('small');note.className='skill-restriction';note.textContent=restriction;el.appendChild(note);}
 ...
 selectSkill(id){
  ...
  if(!(weights[id]>0)&&phaseSkillCount(weights)>=MAX_PHASE_SKILLS){this.toast('使う技を入れ替えるには、選択中の技をひとつ外してください。');return;}
 ...
 describeBelonging(slot){
  ...
  const note=uiEquipmentNote(p,this.g.nearRack()),slots=[
   ...
   {id:'shield',label:'左手',name:p.shield?'木鉄の盾':'装備なし',glyph:'shield',desc:note},
   ...[0,1].map(i=>{const id=p.inventory[i],item=ITEMS[id];return {id:'item'+i,label:'手荷物 '+(i+1),name:item?.name||'空き',glyph:item?.icon||'bag',desc:item?.desc||'拾った品物を持ち歩けます。手荷物は2つまでです。',item:i,filled:!!item};})
  ];
 ...
 hints(){
  if(!this.g.snapshot)return;
  this.open('hints','旅の道しるべ','<div id="journey-hints"></div>');this.paintHints(this.g.snapshot);
 }
 paintHints(s){
  const node=document.getElementById('journey-hints');if(!node)return;
  const hints=uiJourneyHints(s);UIValue.html(node,hints.map(h=>`<article data-hint="${h.id}"><span class="hint-seal" aria-hidden="true">${icon(h.glyph)}</span><div><h3>${ESC(h.title)}</h3><p>${ESC(h.text)}</p></div></article>`).join(''));
 }
 worldSpeedSettings(){
 ...
 event(e){
  ...
  if(e.type==='insight'||e.type==='passive'){const sk=skillById(e.id);if(sk)this.notify({key:'discovery:'+e.type,discoveryKind:e.type,parts:[sk.name],priority:1});}
  ...
  if(own&&['downed','rescueLift','rescueSafe','revived'].includes(e.type))this.notify({key:e.type,parts:[{downed:'倒れて動けない。助けを待とう',rescueLift:'安全な場所へ運んでもらっている',rescueSafe:'安全な場所に着いた',revived:'立ち上がれるようになった'}[e.type]],priority:4});
  if(e.type==='notice')this.toast(e.text);if(e.type==='pickup')this.toast((ITEMS[e.item]?.name||'品物')+(e.gift?'を受け取った':'を拾った'));if(e.type==='depart')this.toast('最前線に到着した');if(e.type==='returned')this.toast('村に戻った');
 }
 update(s,world=true){
  ...
  const fatigue=(this.staminaFeedback??=new UIStaminaFeedback()).sample(p,performance.now()/1000);UIValue.attr(q,'data-fatigue',fatigue.level);if(fatigue.sound&&!this.modal)this.g.audio.fx?.({type:'breath',player:p.id,x:p.x,z:p.z});
  const orbClass='stamina-orb'+(fatigue.level?' tired':'')+(p.seated?' recovering':'');UIValue.attr(q,'class',orbClass);
  ...
  this.updateShipNotice(s);this.updateContext(s);if(world)this.updateWorld(s);if(this.modal==='hints')this.paintHints(s);
 ...
 updateContext(s){const p=s.player,t=s.t,school=nearbyActivity(p,{...s.room,map:s.map}),item=s.room.items?.find(i=>i.ready<=t&&Math.hypot(i.x-p.x,i.z-p.z)<2.3),dummy=s.actors.filter(a=>a.kind==='dummy'&&a.alive&&Math.hypot(a.x-p.x,a.z-p.z)<4).sort((a,b)=>(b.id===p.autoFight)-(a.id===p.autoFight)||dist(a,p)-dist(b,p))[0],options=[];
  if(!p.prologue&&canAct(p)&&!p.rescueTarget){
   ...
   if(atClinic(p,s.room.kind==='village'?{kind:'village',map:s.map}:null))options.push({id:'treatment',name:p.seated?'立ち上がる':'手当てを受ける',glyph:p.seated?'close':'hand',facility:'clinic'});
   const casualty=[...s.players,...s.actors.filter(a=>a.kind==='guard')].find(q=>q.id!==p.id&&q.alive&&q.lifeState==='downed'&&!q.carrierId&&Math.hypot(q.x-p.x,q.z-p.z)<=LIFE_RULES.rescueRange);
   if(casualty)options.push({id:'rescue',value:casualty.id,name:casualty.name+'を救助',glyph:'hand',anchor:{id:'rescue:'+casualty.id,kind:casualty.kind==='player'?'player':'actor',target:casualty.id,height:1.15}});
 ...
 contextAvailable(o,s){
  ...
  if(o.id==='treatment')return atClinic(p,{kind:s.room.kind,map:s.map});
  if(o.id==='activity'){const a=nearbyActivity(p,{...s.room,map:s.map});return !!a&&a.id===o.facility&&p.age>=4;}
  ...
  if(o.id==='rescue'){const a=[...s.players,...s.actors.filter(a=>a.kind==='guard')].find(a=>a.id===o.value);return !!a&&a.id!==p.id&&a.alive&&a.lifeState==='downed'&&!a.carrierId&&Math.hypot(p.x-a.x,p.z-a.z)<=LIFE_RULES.rescueRange;}
 ...
 activateContext(id,value){
  ...
  if(id==='rack'){this.rack();return;}this.g.stopInput();if(id==='treatment')this.g.command({type:'treatment'});if(id==='activity')this.g.command({type:'activity',activity:value});if(id==='return')this.g.command({type:'return'});if(id==='rescue')this.g.command({type:'rescue',target:value});
 ...
 updateWorldLabels(s,now=performance.now()){
  ...
  if(!(pain&&t<pain.until)&&this.staminaFeedback?.level&&now/1000<this.staminaFeedback.breathUntil&&!(p.speech&&p.speechUntil>t)&&!this.modal)add('breath:'+p.id,'breath-callout',p,head(p),'はぁ、はぁ…','',p.combo?-66:-22);
