...
class UI {
 ...
 death(p){if(this.deathShown===p.id)return;this.deathShown=p.id;this.g.stopInput();setTimeout(()=>{if(this.g.snapshot?.player.id!==p.id||p.alive)return;const sk=skillById(p.bankedSkills?.[0]);this.open('death','命の灯',`<div class="death-leaf">${icon('leaf')}</div><h3 class="death-name">${ESC(p.name)}</h3><p class="death-age">享年 ${Math.floor(p.age)}歳</p><p>${sk?`「${ESC(sk.name)}」を、系譜に刻んだ。`:'静かな灯が、系譜に残った。'}</p>${SkillPresentation.recap(p)}<button id="next-life" class="begin-button">${label('次の命へ')}${icon('arrow')}</button><button class="full-button" id="view-lineage">系譜をひらく</button>`);document.getElementById('next-life').onclick=()=>this.g.toClan();document.getElementById('view-lineage').onclick=()=>this.lineage();},1600);}
 event(e){
 ...
 updateWorldLabels(s){
  ...
  this.floatLines=this.floatLines.filter(e=>t-e.born<(e.life??4.6));
  const latest=new Map();for(const e of this.floatLines)if(e.type!=='speech')latest.set(e.player,e);
  for(const e of latest.values()){const a=s.players.find(a=>a.id===e.player);if(a&&Math.hypot(a.x-p.x,a.z-p.z)<14)add('progress:'+e.player,'progress-float'+(e.type==='skillconnection'?' skill-connection':e.type==='skillglimpse'?' skill-glimpse':''),a,head(a)+.3,e.text,'',p.combo?-125:-40);}
