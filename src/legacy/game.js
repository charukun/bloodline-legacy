...
class Game{
 ...
 installInput(){
  ...
  window.addEventListener('keyup',e=>{const wasMove=['KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.code);this.keys.delete(e.code);if(wasMove&&!this.ui.modal&&!this.keys.size&&!this.pointer)this.command({type:'move',x:0,z:0});});window.addEventListener('blur',()=>this.stopInput());
 }
