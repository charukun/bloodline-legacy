...
class Game{
 ...
 loadMode(){
  ...
  this.playerId=[...this.sim.players.values()].find(p=>p.owner===this.profile.owner&&p.alive)?.id||[...this.sim.players.values()].filter(p=>p.owner===this.profile.owner).at(-1)?.id||null;
 ...
 async importSave(file){
  ...
  try{const j=JSON.parse(await file.text());if(j.format!=='AERIN-portable-1')throw Error('血脈の系譜の記録ではありません');
   ...
   this.playerId=[...sim.players.values()].find(p=>p.owner===profile.owner&&p.alive)?.id||null;
