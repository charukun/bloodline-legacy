...
class Game{
 ...
 async importSave(file){if(!file)return;if(this.online){this.ui.toast('共有の旅は、サーバーに保存される');return;}if(file.size>12000000){this.ui.toast('記録が大きすぎる');return;}try{const j=JSON.parse(await file.text());if(j.format!=='AERIN-portable-1')throw Error('血脈の系譜の記録ではありません');const sim=Simulation.restore(j.world);if(!confirm('この記録に切り替えますか？ 今の旅はバックアップします。'))return;writeStore('backup.'+Date.now(),{profile:this.profile,world:this.sim.exportState()});this.stopInput();this.sim=sim;this.profile={...this.profile,...j.profile,online:false,mode:sim.mode,settingsVersion:5};this.loadedMode=sim.mode;this.playerId=[...sim.players.values()].find(p=>p.owner===this.profile.owner&&(p.alive||p.legacyChoice?.state==='pending'))?.id||null;this.snapshot=this.playerId?this.decorate(sim.snapshot(this.playerId,sim.seq)):null;this.seq=sim.seq;this.saveWorld();this.toClan();this.ui.toast('旅の記録を読み込んだ');}catch(e){this.ui.toast(e.message);}}
