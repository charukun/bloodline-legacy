...
const SkillPresentation = (() => {
 ...
 function paint(ui){
  ...
  el.innerHTML=`<span class="revelation-rays" aria-hidden="true">${Array.from({length:12},(_,i)=>`<i style="--ray:${i};--angle:${i*30}deg"></i>`).join('')}</span><svg class="revelation-crest" viewBox="0 0 260 90" aria-hidden="true" fill="none" stroke="currentColor"><path d="M6 47 58 38 82 45M254 47 202 38 178 45M24 57 68 49 82 53M236 57 192 49 178 53"/><ellipse cx="130" cy="44" rx="49" ry="34"/><ellipse cx="130" cy="44" rx="44" ry="30" stroke-opacity=".45"/><path d="m130 2 4 11-4 5-4-5Zm0 68 4 5-4 12-4-12Z" fill="currentColor"/></svg><strong class="revelation-word">閃き</strong><span class="revelation-name">${ESC(names.slice(0,2).join('・'))}${names.length>2?` ＋${names.length-2}`:''}</span>`;
