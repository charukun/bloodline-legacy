...
function renderScene(motion=false){const {record:r,skill:s,skillId}=active(),empty=!r&&!s,valid=available(skillId),current=model.current;
 ...
 $('#inherit-action').hidden=!!current||empty;$('#inherit-action').disabled=!valid;$('#inherit-action').setAttribute('aria-pressed',String(valid&&draft.inherit===skillId));$('#inherit-action span').textContent=!valid?'継承できる技の記録なし':'この経験を継ぐ';
...
function syncCommand(){
 ...
 $('#command-eyebrow').textContent=c?.legacyChoice?.state==='pending'?'この人生から、後世へ':c?'今の人生を、そのまま':valid?'この経験を、次の人生へ':model.skills.length?'一族に残された経験':'白紙から始まる物語';
 ...
 $('#begin-life').textContent=c?.legacyChoice?.state==='pending'?'遺す技を選ぶ':c?(options.inGame?'ゲームへ戻る':'旅を続ける'):valid?'この記憶を継ぐ':model.skills.length?'記憶を探す':firstLife()?'最初の命を迎える':'次の命を迎える';
