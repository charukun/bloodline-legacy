...
function renderScene(motion=false){
 ...
 $('#skill-note').textContent=living?'これまでの経験を携えて、今の人生を続けます。':s?.desc||(view.history?'技が残らなかった人生も、一族の歩みに残ります。':model.skills.length?'技を選ぶと、その効果と記憶を残した人生がここに現れます。':'継承できる技はまだありません。次の人生で、新しい経験を重ねられます。');
 $('#inherit-effect').hidden=!!living||view.history||!s;
 $('#inherit-effect').textContent='受け継ぐのは経験。次の人生の閃きに影響します。技の習得は、その人生で。';
...
function syncCommand(){
 ...
 if(c){$('#command-eyebrow').textContent='今を生きる';$('#command-name').textContent=c.name||'続いている旅';$('#begin-life').textContent=options.inGame?'ゲームへ戻る':'旅を続ける';return;}
 if(isPrologue()){$('#command-eyebrow').textContent='最初の命 · '+races[draft.race].name;$('#command-name').textContent=draft.name.trim()||'風にまかせた名前';$('#begin-life').textContent='人生を始める';return;}
