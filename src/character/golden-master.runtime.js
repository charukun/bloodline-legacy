...
const CM01 = (()=>{
 ...
 class Character {
  ...
  groundAt(x,z){
   ...
   if(this.groundKey!==r.sceneKey){this.groundKey=r.sceneKey;this.groundCells=new Map();for(const [type,rows]of r.static){if(!['rbox','box','softbox'].includes(type))continue;for(const m of rows){const sx=Math.hypot(m[0],m[2]),sz=Math.hypot(m[8],m[10]),sy=Math.abs(m[5]);if(m[13]<.06||m[13]>.32||sy>.26||sx>8||sz>8)continue;const radius=(sx+sz)*.5;for(let ix=Math.floor(m[12]-radius);ix<=Math.floor(m[12]+radius);ix++)for(let iz=Math.floor(m[14]-radius);iz<=Math.floor(m[14]+radius);iz++){const key=ix+','+iz;if(!this.groundCells.has(key))this.groundCells.set(key,[]);this.groundCells.get(key).push(m);}}}}
