...
const Travelers = (()=>{
 ...
 void main(){vec3 facePos=pos,faceNor=nor;travelerFace(facePos,faceNor,region);mat4 m=bone(int(joints.x))*weights.x+bone(int(joints.y))*weights.y+bone(int(joints.z))*weights.z+bone(int(joints.w))*weights.w;vec4 w=m*vec4(facePos,1.);vWorld=w.xyz;vNormal=normalize(mat3(m)*faceNor);vInk=color;vSurface=surface;vLocal=pos;vUv=uv;vRegion=int(region+.5);vShadow=lightVP*w;gl_Position=vp*w;}`;
 const regionHeader=`in vec2 vUv;flat in int vRegion;uniform vec4 cmLoss;uniform float cmArmor;uniform vec4 cmFace;
 bool hidden(){return ((vRegion==22||vRegion==23)&&cmFace.x<.3)||(vRegion==2&&cmLoss.x>.5)||(vRegion==3&&cmLoss.y>.5)||(vRegion==4&&cmLoss.z>.5)||(vRegion==5&&cmLoss.w>.5)||(vRegion==10&&cmArmor<.5)||(vRegion==11&&(cmArmor<1.5||cmLoss.x>.5))||(vRegion==12&&(cmArmor<1.5||cmLoss.y>.5));}`;
 ...
 class Character {
  ...
  update(source,t){
   ...
   const rootQ=Q.euler((authored?0:pose.pitch)+reaction.pitch+ail.pitch+injury.pitch+fall*1.48,(p.dir||0)+(authored?0:pose.yaw),(authored?0:pose.roll)+reaction.roll+ail.roll+injury.roll);
