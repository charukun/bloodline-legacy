...
const SkillComposition=(function createSkillComposition(){
 ...
 const feet=[
  {code:0,key:'plant',name:'据',distance:0,angle:0,text:'足場を保つ。'},
  {code:1,key:'drive',name:'踏',distance:.85,angle:0,text:'深く踏み込む。'},
  {code:2,key:'side',name:'廻',distance:.6,angle:Math.PI/2,text:'横へ足を運び、攻める軸をずらす。'},
  {code:3,key:'draw',name:'退',distance:.45,angle:Math.PI,text:'半歩引いて間合いを測る。'}
 ];
 const reactions=[
  {code:0,key:'break',name:'崩',action:{stagger:.65},entry:'offbalance',fx:'fracture',text:'姿勢を崩す。'},
  {code:1,key:'bind',name:'縫',action:{status:'slow',duration:1.4},entry:'close',fx:'pinch',text:'命中した相手の動きを鈍らせる。'},
  {code:2,key:'drain',name:'削',action:{status:'weak',duration:1.2},entry:'rhythm',fx:'cleave',text:'短く相手の力を削ぐ。'},
  {code:3,key:'push',name:'押',action:{knockback:.48},entry:'offbalance',fx:'ripple',text:'打点から相手を押し離す。'}
 ];
 const endings=[
  {code:0,key:'follow',name:'追',phase:0,distance:.35,angle:0,exit:'close',recovery:.55,text:'打ち終わりに詰め、近さを残す。'},
  {code:1,key:'settle',name:'繋',phase:1,distance:0,angle:0,exit:'rhythm',recovery:.28,text:'その場で構えを整え、次の拍子へ渡す。'},
  {code:2,key:'withdraw',name:'離',phase:2,distance:.65,angle:Math.PI,exit:'rhythm',recovery:.65,text:'打ち終わりに引き、間合いを開ける。'}
 ];
 ...
 function build(input,anchor){
  ...
  return {id:skillId,key:`bl.skill.v1.${f.key}.${c.key}.${w.key}.${r.key}.${e.key}`,family:f.family,phase,
   names:{ja:`${w.name}・${f.name}${c.name}〈${r.name}${e.name}〉`,en:`${f.key} ${c.key} ${w.key} ${r.key} ${e.key}`},
...
class Simulation {
 ...
 moveAttackStep(p,r,dx,dz,slide=false){
  ...
  const bounded={x:0,z:0,supportHeight:p.supportHeight||0},bodyRadius=this.collisionRadius(p);
 ...
 beginComboStrike(p,forcedId=null){
  ...
  p.attackStep=approachDistance?{started:this.time+Math.max(0,timing.charge-(approach?.2:.12)),until:this.time+Math.max(.001,timing.charge),distance:approachDistance,progress:0,moved:0,dir:p.dir+(approach?.angle||0),blocked:false,sounded:false}:null;
...
inflictWound(p,part,severity,source=null,strength=null){
 ...
 tickActor(e,r,ps,dt){
  ...
  if(distance>reach*.82){const speed=(e.kind==='guard'?2.8:e.kind==='boss'?2.6:3.1)*injury.move*(hasStatus(e,'slow',this.time)?.5:1)*(hasStatus(e,'root',this.time)?0:1);this.moveAttackStep(e,r,Math.sin(e.dir)*speed*dt,Math.cos(e.dir)*speed*dt,true);e.action='run';e.guard=false;if(r.kind==='village')e.z=Math.min(-29.6,e.z);}
