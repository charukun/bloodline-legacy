...
function makeVillage(seed=1,terrainRevision=1) {
 ...
 return {seed,terrainRevision,houses,traversables,clinic:GATE_CLINIC,schools:SCHOOLS.map(x=>({...x,x:x.x*mirror,z:x.z+(x.id==='church'?0:(rng()-.5)*1.2)})),port:{x:0,z:28},gate:{x:0,z:-29}};
}
...
class Simulation {
 ...
 spawnVillageNPCs(r){
  ...
  const spot=villagePracticePosition(r.map),dummy=this.actor('dummy',spot.x,spot.z);dummy.name='稽古人形';r.actors.push(dummy);this.ensureClinicStaff(r);
 }
 ensureClinicStaff(r){
 ...
 moveAttackStep(p,r,dx,dz,slide=false){
  ...
  const sceneryBlocked=q=>{
   ...
   if(from>TRAVERSAL_RULES.walkStep&&!stairAt(r.map,p.x,p.z)&&!stairAt(r.map,q.x,q.z)){
 ...
 damageActor(e,source,part,power,r){
  if(!canAct(e)||power<=0)return;power*=combatMaturity(source);const awareness=source?this.awareness(e,source):{engaged:false,unaware:false};
