...
class Simulation {
 ...
 moveAttackStep(p,r,dx,dz,slide=false){
  ...
  const sceneryBlocked=q=>{
   ...
   // A lunge, recovery step or disabled carrier cannot walk off a retaining wall.
   if(from>TRAVERSAL_RULES.walkStep&&!stairAt(r.map,p.x,p.z)&&!stairAt(r.map,q.x,q.z)){
    const radius=this.collisionRadius(p);for(const [x,z] of [[-radius,0],[radius,0],[0,-radius],[0,radius]])if(supportHeight(r.map,q.x+x,q.z+z)<from-TRAVERSAL_RULES.walkStep)return true;
   }
   q.supportHeight=floor;const bounded={...q};this.bound(bounded,r,this.collisionRadius(p));return Math.hypot(bounded.x-q.x,bounded.z-q.z)>1e-6;
  };
