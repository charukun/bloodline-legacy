class Renderer{
 ...
 this.setupSurfaceUniforms?.(options);this.drawBatches(this.static,this.program);this.drawBatches(this.dynamic,this.program);gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);gl.depthMask(false);this.drawBatches(this.groundFX,this.program);this.drawBatches(this.fxBatches,this.program);gl.blendFunc(gl.SRC_ALPHA,gl.ONE);this.drawBatches(this.arcaneFX,this.program);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);this.drawWeather?.(snapshot);this.drawBatches(this.impactFX,this.program);gl.enable(gl.DEPTH_TEST);gl.depthMask(true);gl.disable(gl.BLEND);
 this.diorama.render();
