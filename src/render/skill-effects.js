...
const SkillEffects = (() => {
 ...
 const presetData=[
  ['薄月','blade','sweep','single','cleave','vanish','弧を描く薄い斬面。先端だけが走り、余韻を残さず切れる。'],
 ...
 const duration=1.5;
 function frame(recipe,time,quality='high'){
  if(!Number.isFinite(time))return [];
  const list=[];
  for(const [i,at] of beats(recipe).entries()){
   const u=(time-(at-.18))/.23,mirror=i%2?-1:1;
   for(const p of stroke(recipe,u,quality))list.push(transform(p,v=>[v[0]*mirror,v[1],v[2]]));
 ...
 function transform(p,point){return p.kind==='line'?{...p,a:point(p.a),b:point(p.b)}:{...p,p:point(p.p)};}
