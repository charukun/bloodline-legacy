...
vec3 localLight(vec3 p,vec3 intensity,vec3 base,vec3 N,vec3 V,float rough,float metal){if(!any(greaterThan(intensity,vec3(0.))))return vec3(0.);vec3 delta=p-vWorld;float d=length(delta),atten=pow(max(0.,1.-d/7.5),2.)/(1.+d*d*.12);return brdf(base,N,V,delta/max(.01,d),rough,metal,intensity*atten);}
...
void main(){vec3 c=decode(texture(image,uv).rgb);float d=texture(depth,uv).r;float occ=0.;
 if(d<.9999){for(int k=0;k<8;k++){float a=float(k)*2.399963;float r=2.+float(k%4)*3.5;vec2 off=vec2(cos(a),sin(a))*r*texel;float delta=d-texture(depth,uv+off).r;occ+=smoothstep(.0002,.0024,delta)*(1.-smoothstep(.004,.016,delta));}c*=1.-occ*.060*occlusion;}
 ...
 // Edge-only anti-aliasing. No depth-of-field, no full-frame softening.
 vec3 a=aces(decode(texture(image,uv+vec2(texel.x,0.)).rgb)*exposure),b=aces(decode(texture(image,uv-vec2(texel.x,0.)).rgb)*exposure);float contrast=length(a-b);if(contrast>.40)c=mix(c,pow((a+b)*.5,vec3(1./2.2)),.16);
