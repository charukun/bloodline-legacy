...
const EnemySentinel=(()=>{
 ...
 const settings=p=>p.kind==='elite'?{scale:[1.91,1.86,1.82],cloth:[.64,.29,.24],metal:[.87,.81,.69],stride:1.8,impact:.61}:{scale:[1.34,1.42,1.34],cloth:[.29,.48,.45],metal:[.68,.79,.78],stride:1.65,impact:.61};
 ...
 function pose(p,t,rec={}){const config=settings(p),motionT=p.renderPoseTime??t,dt=Math.max(0,Math.min(.1,motionT-(rec.time??motionT))),distance=Math.hypot(p.x-(rec.x??p.x),p.z-(rec.z??p.z));
  ...
  const matrices=local.map(n=>trs(n.translation,n.rotation,n.scale)),world=[];function node(i){if(world[i])return world[i];return world[i]=asset.parent[i]<0?matrices[i]:rMultiply(node(asset.parent[i]),matrices[i]);}for(let i=0;i<local.length;i++)node(i);
  ...
  Object.assign(rec,{time:motionT,x:p.x,z:p.z,local,world,palette,sockets,timing,config,root,dt});return rec;
 }
 ...
 const top=p=>p.kind==='elite'?4.8:3.5;
